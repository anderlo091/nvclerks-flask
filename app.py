...
def validate_env_vars():
    required_vars = {
        ...
        "ENCRYPTION_KEY": lambda x: len(base64.b64decode(x)) == 32,
        "HMAC_KEY": lambda x: len(base64.b64decode(x)) == 32,
    ...
    for var, validator in required_vars.items():
        ...
        if not value or not validator(value):
            ...
            raise ValueError(f"Environment variable {var} is missing or invalid")
...
USER_TXT_URL = os.getenv("USER_TXT_URL")
DATA_RETENTION_DAYS = int(os.getenv("DATA_RETENTION_DAYS"))
...
TRUSTED_USER_AGENTS = ["Chrome", "Firefox", "Safari", "Edge"]
...
def is_bot(user_agent, headers, ip, endpoint):
    try:
        ...
        if endpoint.startswith("/") and endpoint != "/login":
            logger.debug(f"IP {ip} accessing generated link {endpoint}")
            ua = parse(user_agent)
            if ua.browser.family in TRUSTED_USER_AGENTS:
                logger.debug(f"Trusted user-agent {ua.browser.family} for IP {ip}, bypassing JS verification")
                return False, "Trusted browser"
            session_key = f"user:session:{ip}"
            js_verified = valkey_client.get(session_key)
            if js_verified == "1":
                logger.debug(f"JS verification found in Valkey for IP {ip}")
        ...
        logger.warning(f"Blocked IP {ip}: Unknown reason")
        return True, "Unknown"
    except Exception as e:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to write to /tmp/geoip_log.txt: {str(e)}")
    ...
    def decorator(f):
        ...
        def wrapped_function(*args, **kwargs):
            try:
                ...
                effective_limit = bot_limit if is_bot_flag else limit
    ...
    try:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            if not error:
                ...
                if not error:
                    generated_url = f"https://{urllib.parse.quote(subdomain.lower())}.{base_domain}/{endpoint}/{url_id}/{urllib.parse.quote(encrypted_payload, safe='')}/{urllib.parse.quote(path_segment, safe='/')}"
                    max_retries = 2
                    for attempt in range(max_retries):
                        try:
                                            ...
                                            <div>
                                                ...
                                                <script>
                                                    try {
                                                        ...
                                                        console.error('Link chart error:', e);
                                                    }
                                                </script>
                                            </div>
                                            <div>
...
@app.route("/<endpoint>/<url_id>/<path:encrypted_payload>/<path:path_segment>", methods=["GET"], subdomain="<username>")
@rate_limit(limit=5, per=60, bot_limit=2)
def redirect_handler(username, endpoint, url_id, encrypted_payload, path_segment):
    try:
        base_domain = "nvclerks.com"
        user_agent = request.headers.get("User-Agent", "")
        ip = request.remote_addr
...
@app.route("/<endpoint>/<url_id>/<path:encrypted_payload>/<path:path_segment>", methods=["GET"])
@rate_limit(limit=5, per=60, bot_limit=2)
def redirect_handler_no_subdomain(endpoint, url_id, encrypted_payload, path_segment):
    try:
        host = request.host
        username = host.split('.')[0] if '.' in host else "default"
        logger.debug(f"Fallback redirect handler: username={username}, endpoint={endpoint}, url_id={url_id}, encrypted_payload={encrypted_payload[:20]}..., path_segment={path_segment}, URL={request.url}")
        return redirect_handler(username, endpoint, url_id, encrypted_payload, path_segment)
    except Exception as e:
